...
import CommonModal from "../dashboard/patternModals/CommonModal";
import { toast, Toaster } from "react-hot-toast";
import SetupCommunicationModal from "../../components/modal/SetupCommunication.jsx";
...
const PatternConfig = ({ onClose }) => {
  ...
  useEffect(() => {
    if (user?.preferred_patterns_details) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Flatten and collect all current pattern names
      const preferredPatterns = Object.values(patterns).flatMap((category) =>
        category.map((pattern) => pattern.name)
      );
      // if (!preferredPatterns?.length)
      //   throw new Error("Please select atleast one of pattern name to save.");
      ...
      // Refetch patterns to ensure consistency
      setIsSetModalOpen(true);

      // Use setTimeout to ensure modal opens after a short delay
      setTimeout(() => {
        setIsSetModalOpen(true);
      }, 300);
    } catch (error) {
  ...
  const handleSetupCommunication = (communicationData) => {
    // Add any additional logic for handling communication setup
    handleCloseSetModal();
  };
  return (
    <div className="p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <SetupCommunicationModal
          ...
          onSubmit={handleSetupCommunication}
        />
      </div>
