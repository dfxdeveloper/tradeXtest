...
export default function TalkToExpertModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post("user/talktoexpert", formData);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-[linear-gradient(359.93deg,_#220C39_-10.38%,_#6C4984_99.93%)] rounded-3xl border border-[#B039FF] shadow-xl p-4 w-full max-w-md relative">
        ...
        <h2 className="text-xl font-bold mb-6 text-white">Talk to Expert</h2>
        <form className="space-y-4 px-4" onSubmit={handleSubmit}>
          {[
            ...
            { name: "subject", label: "Subject", type: "text", placeholder: "Enter your Subject" },
          ].map((field) => (
            <div key={field.name}>
