      ...
      groups.map((group) => {
        if (group.id === parentGroupId) {
          const newNestedGroup = {
            type: "group",
            data: {
              id: group.items.filter((item) => item.type === "group").length + 1,
              items: [
                {
                  type: "condition",
                  data: {
      ...
      groups.map((group) => {
        if (group.id === groupId) {
          const newCondition = {
            type: "condition",
            data: {
              id: group.items.filter((item) => item.type === "condition").length + 1,
              type: "Select",
              operator: "AND",
              subType: "",
              option: "",
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-2 sm:p-4 text-white">
      ...
      <div className="mb-6 text-center mt-10">
        <button className="bg-[#6A11CB] hover:bg-purple-600 text-white px-4 py-2 rounded-lg font-medium">
          + Add Group
        </button>
      </div>
      <div className="bg-[#6A11CB] text-white p-3 rounded-lg mb-6">
