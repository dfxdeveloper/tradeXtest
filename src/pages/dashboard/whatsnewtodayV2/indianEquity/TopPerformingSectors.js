...
function TopPerformingSectors() {
  ...
  return (
    ...
    <div className="flex items-center mb-4">
      ...
      <h2 className="text-lg font-medium font-gilroy text-[#D5AFFF]">Top Performing Sectors</h2>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {topPerformers.map((item, index) => (
        <div
          key={index}
          className="bg-gradient-to-r from-[#0FEDBE52] to-[#09876C00] border border-[#0FEDBE] rounded-lg p-4"
        >

          <div className="flex justify-between mb-2">
            <div className="font-medium font-gilroy">{item.sector}</div>
            <div className="text-[#00D200] font-gilroy">{item.performance}</div>
          </div>
          <div className="grid grid-cols-2 gap-y-2 font-gilroy text-sm text-white">
            <div>Market Cap:</div>
